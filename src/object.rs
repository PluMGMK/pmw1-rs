...
impl Pmw1Object {
    ...
    /// Add a new [relocation block](Pmw1RelocBlock) to the object. This makes sure that the block
    /// is compressed or uncompressed as appropriate to the state of the object itself. If
    /// [compression](Pmw1RelocBlock::compress) or [decompression](Pmw1RelocBlock::decompress)
    /// fails, this method returns the error generated by that process.
    pub fn add_reloc_block(&mut self, block: Pmw1RelocBlock) -> Result<()> {
        // Make sure the compression state is correct.
        if self.compressed() {
            self.relocation_blocks.push(block.compress()?);
        } else {
            self.relocation_blocks.push(block.decompress()?);
        }
